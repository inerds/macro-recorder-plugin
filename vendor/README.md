...
`@lottiefiles/vite-plugin-creator` is LottieFiles' own Creator-plugin tooling.
The registry carries it only in versions this project cannot use, so it is
installed from a `file:` dep in `package.json` rather than a registry range:
