  ...
  it("spans added, removed and changed frames across steps", () => {
    const steps = [
      buildStep({
        op: "keyframes",
        path: ["position"],
        added: [{ frame: 12, value: 1 }],
        removed: [],
        changed: [],
        layer: LAYER,
      } as unknown as StepPayload),
      buildStep({
        op: "keyframes",
        path: ["opacity"],
        added: [],
        removed: [{ frame: 4, value: 0 }],
        changed: [{ before: { frame: 30, value: 0 }, after: { frame: 60, value: 1 } }],
        layer: LAYER,
      } as unknown as StepPayload),
    ];
...
describe("hasKeyframes", () => {
  ...
  it("is false for an empty macro and for opaque payloads", () => {
    ...
    expect(hasKeyframes([{ id: "x", kind: "other" as const, label: "opaque", payload: {} }])).toBe(
      false,
    );
  });
});
