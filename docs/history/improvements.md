...
| **A successful stagger was reported as a skipped step.** Notes were bare strings, so the summary counted "delayed this layer by 4 frames" alongside genuine non-applies. | Notes carry a kind: `NoteList` records `skip` for `push` and `info` for `info`, the kind rides the `playback.step` result (absent on an old sandbox reads as skip), and `summarizePlaybackNotes` counts skips alone, saying "N steps adjusted" when a run only adapted. ENGINE_REV 2026-09-06.4 covers the note-kind and wording changes. |
