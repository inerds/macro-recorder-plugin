...
| The theme relay reached `creator.ui.theme` and `creator.on` through `as unknown as` casts, written when both surfaces were absent from the 0.0.2 typings. | Both are typed in creator-api-types 1.0.1, so the casts are gone and the calls are type-checked. The relay stays feature-detected and every try/catch stays: neither surface is live-verified, so a host without them is still a normal outcome. |
| `initSelectionEvents` reached the event bus through an `AnyProxy` cast and `on.call(creator, …)`. | It now calls the typed `creator.on("selection:keyframes", …)` behind a `typeof` guard. The defensive "bare array or `{data}` envelope" payload read stays, because the runtime payload shape is still unverified. |
| Paint, stroke and mask removal first probed for `container.removeFill`/`removeStroke`/`removeMask`, and the fake scene implemented all three — methods the real host has never had. | The probe is deleted; removal is the entry's own `.remove()` with the list length re-read so a no-op reports a miss. The three methods are gone from the fake scene, which no longer offers a surface the host lacks. |
...
| **Playback progress announced every step, up to ~20 times a second**, so a screen reader heard nothing else during a run. | The progress sentence is aria-hidden; a run announces "Playing N steps…" once at the start and its outcome once at the end through the panel's notice live region. |
| Simplify on the review sheet said nothing: the reducer dropped every NOTICE outside idle and recording. | The reviewing state carries a notice too, gated by `hasNoticeChannel()`, so the merge count is spoken and toasted there. |
| A failed sandbox handshake left the panel blank for the whole session: `ui/main.tsx` had no rejection handler. | The catch renders the panel on mock gateways with the demo-engine banner, and falls back to a static alert line if even that fails. |
| The capture keys' reasons were unreachable: a natively disabled key leaves the tab order and the library kills the pointer events its tooltip needs. Play and play-options went dead mid-run the same way. | All of them use `aria-disabled` with a guarded click, a title, and an sr-only reason ("Another macro is playing"), the `SimplifyButton` idiom. |
| The playback report, the only account of what a run adapted or skipped, timed out after 3 seconds. | Notices carrying playback notes get an 8-second toast; errors still wait for their reader. |
| The deck counter's aria-label sat on a bare span and said "Steps captured" all through playback; the toast covered the recording and review footers; "No macros yet" flashed before the store answered; the row disclosure was ~16px tall; Save, Discard, Cancel and Play dropped focus to the body. | An sr-only label follows the deck state; the toast padding sits on one wrapper for every screen; idle carries a `loaded` flag and the empty copy stays invisible until the store answers; the disclosure stretches to a 22px floor; each sheet hands focus back to its row or the Record key. |
| A paused playback's Continue/Stop could sit off screen; the capture offer was never mentioned; "at playhead" was explained only inside the dialog; the deck said PLAYING beside a dead Stop key; the pre-run Dismiss wore the red cap. | The warn box scrolls into view on pause; the empty feed names the offer; the badge tooltip carries the dialog's sentence from one shared `atPlayheadHint()`; the deck's Stop works during playback; Dismiss wears the cream outline. |
| **A successful stagger was reported as a skipped step.** Notes were bare strings, so the summary counted "delayed this layer by 4 frames" alongside genuine non-applies. | Notes carry a kind: `NoteList` records `skip` for `push` and `info` for `info`, the kind rides the `playback.step` result (absent on an old sandbox reads as skip), and `summarizePlaybackNotes` counts skips alone, saying "N steps adjusted" when a run only adapted. |
| Notes spoke to a developer: "the host kept startFrame unchanged — the write didn't take". | Every read-back note uses Creator's words: "Creator kept the in point as it was — the change didn't apply"; `engine/labels.ts` maps `startFrame`/`endFrame` to "in point"/"out point". |
| "Scene script" was undefined jargon in the row hint, a stagger note, and the guide. | One term, scene rebuild, in the terminology table; the hint reads "Rebuilds the scene — finds N layers by name" and the note "stagger needs layers selected — replayed without it". |
| The guide quoted a play-options badge the panel never emitted; the badge mixed capitalisation; the stagger field said "1 frames"; Discard was asked two ways; failure toasts came in three styles; the capture keys never said what they added; the card read "Duration 30 fr". | One lowercase readout "repeat ×8 · stagger 4 frames · at playhead", pluralised frames, one Discard wording with the key "Discard recording", every fixed failure toast "Couldn't <verb> the macro. Try again.", "Add all keyframes" / "Add selected keyframes (n)", and "Duration 30 frames". |
| **The deck's clock dropped to 8px and the state word to 8.5px** exactly where they mattered most. | Both hold a 9px floor; width comes from padding and tracking instead. |
| **The red key's legend failed 4.5:1 on the top of the cap** (3.43:1 at rest, 3.26:1 on hover). | The face starts at `--primary` and only darkens: rest 4.76 / 5.52 / 6.44:1, hover 4.62 / 5.11 / 5.67:1, pressed 6.30 / 7.06 / 7.93:1; `index.css` and `deck.css` moved together. |
| **Pending rows sat at 0.55 opacity (2.18 to 3.35:1) and the running numeral pulsed down to 2.85:1.** | Pending rows demote by colour, not alpha (`--muted-foreground`, 4.96:1); the running numeral is `--ink-red-text` (5.19:1) and its pulse animates the glow only. |
| The action lane snapped while its buttons eased; quiet-key icons were drawn heavier than their legends; the import placeholder was 2.37:1; the destructive confirm wore the Save key; the running row's tint was 1.01:1; the warn-box border was 1.56:1; the plate had no edge on a light host; small type rendered with subpixel fringing. | The lane shares the buttons' curve; icons use stroke 2 at the skin's 12px; placeholder at full `--muted-foreground` (4.96:1); destructive confirms use `.key-armed` (5.61:1); the running row uses `bg-muted` with the inset bar as the cue; the warn-box edge is amber mixed with ink (3.14:1); `.panel-root` draws a 1px `--border` hairline; `.host-frame` sets antialiased smoothing. |
| The library stylesheet fetches a Google font the skin never renders. | Blocked, with evidence in `docs/design-system.md`: the library's dialogs, menus and toasts are styled with `animate-in` utilities that only its own build generates, so dropping the stylesheet in favour of `@source` scanning removes every entrance. Unblocking needs the animate plugin as a dependency. |
| The panel's fixed "Could not save macro" toast discarded the store's message. | The toast shows the store's own words; the fallback is "Couldn't save the macro. Try again." ENGINE_REV 2026-09-06.4 covers the note-kind and wording changes. |
...
| `remove-paint` and `replace-paint` on a text layer read `container.fills` / `container.strokes`. A text layer keeps ONE fill and ONE stroke as singular accessors, so removal always missed and replay claimed it had "added the new one alongside". | `removeListEntry` falls back to the singular `fill` / `stroke` accessor's own `remove()` at index 0, and verifies by re-reading the accessor. |
| `readBaseline` always returned `prop.staticValue`. On an animated property that value is a stale leftover, so relative playback measured the target against something the user never sees. | With keyframes present, `readBaseline` returns `toJson(prop.getValueAt(creator.timeline.currentFrame))`, guarded, falling back to `staticValue`. |
| The applier probed `container.addFill` / `addStroke` before `createFill` / `createStroke`. Those container members never existed at runtime and are gone from 1.0.1, so the probe only added a misleading branch. | Creation goes through `createFill` / `createStroke` alone. |
| `engine/testing/fakeScene.ts` was more permissive than the host: `addFill`/`addStroke`, ShapeContainerMixin and masks on every node type, paint `opacity`, trim `mode`, writable keyframe tangents, `Size` as `{x,y}`, plain-object `pathData`, `SCENE_INSTANCE`, and no input validation. | The fake now mirrors 1.0.1: type-gated mixins, singular text `fill`/`stroke`, `getValueAt`, getter-based `pathData`, refused tangent writes, and `✗ Invalid input` validators for easings, colors, mask modes, text alignments, paint types and unknown `create*` keys. |
| The dev bundle shipped the release plugin id, so Creator scoped both builds to ONE clientStorage store — a tester who cleared the dev store from the dev strip wiped the macros they had recorded for real. | `vite.config.ts` now stamps a second fixed uuid (`DEV_PLUGIN_ID`) onto `manifest.id` on a `--mode development` build, giving the dev plugin its own store. |
| The UI accepted `change:theme` and every RPC response from ANY window: a sibling frame or opener could repaint the panel's chrome or resolve a pending call with a forged result. | `index.html`'s head script, `useHostBackground()` and `RpcClient.onMessage` now require `event.source === window.parent`, and both theme consumers run the colour through `CSS.supports("background-color", …)` before it reaches a style declaration. |
| The theme relay dropped `isLight`, so the pre-React head script had to read "dark" out of a theme NAME; a host theme called neither light nor dark left the gutter on its dark fallback. | `sandbox/theme.ts` forwards the host's own `isLight`, and the head script resolves it as a rung between the pushed background token and the themeName regex — the same order `useHostBackground()` already used. |
| A UI that lost the ~600 ms handshake race sat in demo mode for the whole session: the only recovery was the `sandbox-ready` notify, which the sandbox posts before the iframe exists, so Creator drops it. | `createGateways` now keeps calling `hello` in the background after falling back — every 1 s for 20 s, then every 5 s — and reloads onto the real engine as soon as the sandbox answers. Only a framed UI retries; a standalone tab's mocks are the point. |
| Every `clientStorage.set` rejection was reported as "Storage full — delete a macro first", sending the user to delete their own work over failures deleting nothing would fix, and hiding the host's message from the trace. | `sandbox/store.ts` says that only when the host's rejection names the cap, and otherwise rethrows the host's own words. `creator.clientStorage.usedQuota` is feature-detected, cached, and returned from `hello` as `usedQuota`. |
| The inline confirmation took focus with `autoFocus` and never gave it back: dismissing it dropped focus to the document, so keyboard users restarted from the top of the panel. | `ConfirmInline` stashes `document.activeElement` on mount and restores it on unmount when the prompt still owned focus, leaving focus alone if the user had moved on. |
| Thirteen className sites merged Tailwind utilities with template literals, so an override like `w-6` over `w-0` depended on generated-stylesheet order rather than the class list. | Those sites now use the library's `cn()`, whose tailwind-merge resolves the conflicts deterministically. `ThemeProvider` also gets `themeName="vintage"`. |
| A mask's blend mode never recorded: `diffMask` compared only `pathData` and `opacity`, so switching a mask from Add to Subtract produced no step at all. | `Mask.mode` (a plain mutable string on 1.0.1) is diffed and emitted as a `set-plain` step on `["masks", i, "mode"]`, which the applier already resolves and writes; the step reads `Mask · mode add → subtract`. |
| Swapping a gradient between linear and radial recorded nothing: `diffPaint` matched on `kind` alone, so both sides were "gradient" and only the stops/start/end were compared. | A differing `gradientType` now falls through to the existing `replace-paint` branch, the same remove-and-recreate a solid↔gradient swap uses. |
| Scene settings were invisible to the recorder: `SceneSnapshot` carried only `sceneId` and `layers`, so changing the scene's size, background, framerate, duration or name recorded nothing. | `SceneSnapshot.settings` (read defensively; a null background is kept as a real value) is diffed into a new absolute `set-scene` step, applied once per run against `creator.activeScene` with a read-back that reports a discarded write. |
| `TYPE_PROPS` registered `SCENE_INSTANCE`, a 0.0.2 type name the runtime never emits, and had no entry for the real `SCENE_LAYER` or for `IMAGE_LAYER`, so both fell back to probing the union of every property. | The registry names `SCENE_LAYER` and `IMAGE_LAYER` (transform props each); the dead `SCENE_INSTANCE` entry is gone. |
| Replaying an `add-layer` for an image layer built an empty shape shell through `createShapeLayer`. | It skips with "can't re-create an image layer — the recording has no image asset — skipped". |
| `playbackBegin` took `creator.selection.nodes` verbatim, so a selected shape became a playback target and every layer-addressed step resolved against the wrong node or failed. | The selection is filtered to layers (`creator.utils.isLayer` when the host has it, feature-detected; otherwise a `startFrame` read); dropped shapes are reported once as "N selected shapes skipped — macros replay onto layers". |
| Switching scenes mid-recording silently recorded nothing: `record.start` pins `creator.activeScene` and every tick re-serializes that proxy. | Each tick compares the active scene's id to the pinned one and, once, emits a non-replayable step reading `You switched scenes — still recording "…"`, appended after the delta so it cannot mask a silent session. |
| The playback probe read values with the generic `toJson`, so every getter-based `PathData` probed as `{}` on both sides and no path write was verifiable in a trace. | The structural reader `valueToJson` is exported from `sandbox/serialize.ts` and used for the static value and each keyframe value. ENGINE_REV 2026-09-06.3 covers this batch. |
| **A same-scene nest replay with anything selected left an orphaned, empty duplicate of the nested scene** (trace 2026-09-05T16-43-18, rev 2026-09-06.1). The "already exists — using it" adoption ran only when nothing resolved; with a stray selection the host's empty scene layer was verified away and the fallback rebuilt a second "Nested Scene 1" next to the real one, and never cached what it built. | `nest-layers` now looks the recorded nest up first. When the host cannot move the layers and that nest is still live, replay adopts it and says so; the rebuild fallback only runs when no nest exists, and its result is cached like `break-scene`'s. Regression test in `sandbox/playback.test.ts`; ENGINE_REV 2026-09-06.2. |
| **A capture-only recording session still tripped the "recorded nothing" whole-session debug pair** (traces 2026-09-04T03-47-27 and 03-51-20): `record.captureKeyframes` returned real steps but never set `recording.stepped`, which only `collectDelta` set, so the trace claimed the session dropped its diff. | `recordCaptureKeyframes` marks the session stepped when it returns steps. Regression test in `sandbox/recorder.test.ts`. |
