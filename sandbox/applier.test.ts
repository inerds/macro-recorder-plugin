...
describe("add+remove-same-frame keyframe payloads (what pre-fix saved macros contain)", () => {
  // engine/diff.ts used to key keyframes by id, and Creator reassigns a
...
describe("captured-keyframe steps round-trip onto a bare layer", () => {
  ...
  // SNAPSHOT (engine/capture.ts); replaying them on a fresh layer must
  ...
  it("recreates position keyframes with easing on a layer that has none", async () => {
    const { captureKeyframePayloads } = await import("../engine/capture");
    const snapshot = {
      nodeId: "rec-1",
      nodeType: "CONTAINER",
      nodeName: "Bounce",
