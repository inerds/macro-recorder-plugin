 ...
 *
 * The demo macros in `ui/dev/demoMacros.ts` are seed DATA a dev session
 ...
 * global, which only tsconfig.sandbox.json knows about; the macros' shape is
 * asserted in `ui/dev/demoMacros.test.ts`.
 */
...
describe("what each demo macro actually does", () => {
  ...
  // silent half-apply (see sandbox/applier.test.ts), which is why this doesn't
  // show up in the "no failures" check above — but the demo's own seed data
  // (ui/dev/demoMacros.ts:377) still sends the wrong-case "NORMAL", so the
  ...
  // cleanly), not what ui/dev/demoMacros.ts currently does.
